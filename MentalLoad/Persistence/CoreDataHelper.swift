...
fileprivate let logger = Logger(category: "CoreDataHelper")
...
extension NSManagedObjectContext {
    ...
    func save(with contextualInfo: ContextualInfoForSaving) {
        if hasChanges {
            do {
                try save()
                logger.info("Saved for: \(contextualInfo.rawValue)")
            } catch {
                logger.error("\(#function): Failed to save Core Data context for \(contextualInfo.rawValue): \(error)")
            }
        }
    }
}
